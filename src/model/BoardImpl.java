...
public class BoardImpl implements Board {
	...
	public void initTile() {
			Random r = new Random();
			int i = r.nextInt(sideSizeInSquares);
			int j = r.nextInt(sideSizeInSquares);
			System.out.println("Tile créée aux coordonnées :" + i + " - " + j);
			if (nextBoard[i][j] != null) {
				initTile(); 
			} else {
				nextBoard[i][j] = new TileImpl(r.nextInt(2) + 1);
			}
	}
}